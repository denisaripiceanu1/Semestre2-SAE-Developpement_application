...
public class ArticlesTest {
	...
	public void testTomates() {
		...
		assertEquals(GenerationArticles.GenerationTomates(),
				GenerationArticles.générationDeBaseDesTomates().tomatesDeType(TypeTomate.TOMATES));
	}
	...
	public void testTomatesCerises() {
		assertEquals(16,
				GenerationArticles.générationDeBaseDesTomates().tomatesDeType(TypeTomate.TOMATES_CERISES).size());
		assertEquals(GenerationArticles.GenerationTomatesCerises(),
				GenerationArticles.générationDeBaseDesTomates().tomatesDeType(TypeTomate.TOMATES_CERISES));
	}

	@Test
	public void testTomatesCerisesAppariement() {
		List<Tomate> tomates = GenerationArticles.générationDeBaseDesTomates()
				.tomatesDeType(TypeTomate.TOMATES_CERISES);
		for (Tomate tomate : tomates) {
			assertEquals(4, tomate.getTomatesApparentées().size());
		}
	}
	...
	public void testTomatesParCouleur() {
		for (int i = 0; i < Couleur.values().length; i++) {
			List<Tomate> tomates = GenerationArticles.générationDeBaseDesTomates()
					.tomatesDetypeDeCouleur(TypeTomate.TOMATES, Couleur.values()[i]);
			assertTrue(tomates.size() > 0);
			for (Tomate tomate : tomates) {
	...
	public void testTomatesCerisesParCouleur() {
		for (int i = 0; i < Couleur.values().length; i++) {
			List<Tomate> tomates = GenerationArticles.générationDeBaseDesTomates()
					.tomatesDetypeDeCouleur(TypeTomate.TOMATES_CERISES, Couleur.values()[i]);
